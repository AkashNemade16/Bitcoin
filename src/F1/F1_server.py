...
clientPortA = 10001  # client-send A
serverPort2 = 11000  # F2
serverPort1 = 9000  # client-recieve
serverPort = 10000  # F1
...
while 1:
    ...
    if clientAddress[1] == clientPortA and len(message.decode()) == 24:
        # recieving the tx
        tx = modifiedMessage
        print("tx received: " + tx)
        serverSocketF1.sendto(tx.encode(), (serverName, serverPort2))
        ...
        if counter == 4:
            ...
            if (turn % 2) == 1:
                ...
                for transm in f_content:
                    serverSocketF1.sendto(transm.encode(), (serverName, serverPort1))

                # sending block to full node f2
                serverSocketF1.sendto(block.encode(), (serverName, serverPort2))
                print("block: " + block)

    elif clientAddress[1] == clientPortA and message.decode() == "blockchain request":
        print("sending blockchain")
        with open('Blockchain', 'r') as b:
            blockchain = b.readlines()
            for block in blockchain:
                serverSocketF1.sendto(block.encode(), (serverName, serverPort1))
        serverSocketF1.sendto('Blockchain sent'.encode(), (serverName, serverPort1))

    # checking if the request is from F2
    elif clientAddress[1] == serverPort2:
        if len(modifiedMessage) == 232:
            ...
            for tx in txs:
                serverSocketF1.sendto(tx.encode(), (serverName, serverPort1))
        else:
